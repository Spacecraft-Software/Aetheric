...
This repository (`/spacecraft-software/aetheric`) is in **Phase 1 — Foundation** (2026-Q3). Source code exists and compiles; core crates are tested, stub crates are scaffolded.

### What exists now

| Path | Purpose | Status |
|---|---|---|
| `Cargo.toml` / `Cargo.lock` | Workspace manifest — 9 member crates | ✅ Compiles |
| `schemas/*.capnp` | Cap'n Proto Constellation schemas (core + orchestration) | ✅ Validated |
| `crates/aetheric-ipc-types/` | Cap'n Proto ↔ Rust types (events, commands, error) | ✅ Tests pass (9) |
| `crates/aetheric-stratum/` | Persistent rope (crop), snapshot registry, encoding | ✅ Tests pass |
| `crates/aetheric-lumen/` | Structured logging (tracing + jiff, JSON/human) | ✅ Tests pass |
| `crates/aetheric-morpheus/` | IPC broker — codec, listener, connection stubs | 🔄 Stubs compile |
| `crates/aetheric-single-screw/` | FFI bridge — `catch_unwind`, POSIX pipe, 6/18 ABI fns | 🔄 Partial |
| `crates/aetheric-orion/` | Tokio runtime — state, config, channels, task stubs | 🔄 Stubs compile |
| `crates/aetheric-rms/` | RMS Microkernel binary — CLI (clap), main entry | 🔄 Stubs compile |
| `crates/aetheric-nova/` | GPU renderer placeholders (Phase 2) | ⏸️ Placeholders |
| `crates/aetheric-penumbra/` | TTY renderer placeholders (Phase 2) | ⏸️ Placeholders |
| `guile/aetheric/*.scm` | Majestic scaffolding: core, rms-ipc, agent, seraph | 📝 Draft |
| `.github/workflows/ci.yml` | CI workflow (fmt, clippy, build, test, deny, spdx) | 📝 Draft |
| `benches/` | Criterion bench stubs (rope, ipc) | 📝 Draft |
| `fuzz/` | libfuzzer targets (stratum insert/delete, morpheus decoder) | 📝 Draft |
| `Aetheric-Master-PRD-v2.7.md` | Product Requirements Document | ✅ Current |
| `IMPL-Aetheric-v2.7.md` | Implementation Plan | ✅ Current |
| `TODO-Aetheric-v2.7.md` | 268 tasks — Phase 1 partially complete | ✅ Current |
| `AGENTS.md` / `AGENTS-Aetheric-v2.7.md` | Agent guidelines | ✅ Current |

### Build toolchain notes

- **rustc version mismatch:** The environment has multiple rustc versions (1.91.1, 1.95.0). `capnpc` crate requires consistency. Use `export RUSTC=/nix/store/9i7krd5vb8pqzax2gh61kp16kcb4lkyn-rustc-1.91.1/bin/rustc` before all `cargo` commands.
- **clippy:** Requires 1.95.0 clippy-driver; incompatible with 1.91.1. Run `cargo clippy` without the `RUSTC` override if possible, or use the newer toolchain.
- **guile-fibers:** Not installed in this environment. Guile tests require `guile-fibers`.

### What does NOT exist yet

- `librms_ipc.so` (requires SingleScrew completion + cbindgen)
- `tests/ss_bridge_smoke.scm` (blocked on SS completion)
- Morpheus accept loop + dispatch (stubbed with `todo!()`)
- Orion `run()` orchestration (stubbed with `todo!()`)
- `docs/spacecraft-software-exceptions.md`
- Nix flake, NixOS module, Home Manager module
- `CHANGELOG.md`

**Target v1.0:** 2027-Q3. **Current phase:** Phase 1 — Foundation (2026-Q3).
...
### Rust / RMS — Working now
```nu
# Set rustc consistently (see §0 toolchain notes)
export RUSTC=/nix/store/9i7krd5vb8pqzax2gh61kp16kcb4lkyn-rustc-1.91.1/bin/rustc

cargo check
cargo fmt -- --check
cargo test -p aetheric-ipc-types -p aetheric-lumen -p aetheric-stratum
cargo build
```

### Rust / RMS — Planned (Phase 1 validation)
```nu
cargo clippy -- -D warnings                  # blocked on rustc/clippy version match
cargo test --all-features
cargo test --test renderer_parity            # Nova vs Penumbra layout
cargo miri test -p aetheric-single-screw     # SingleScrew unsafe only
RUSTFLAGS="-Zsanitizer=thread" cargo test
cargo bench                                  # criterion: rope_ops, ipc_roundtrip
cargo fuzz run fuzz_stratum_insert           # 1-hour soak target
cargo fuzz run fuzz_stratum_delete
cargo fuzz run fuzz_morpheus_decoder
```

### Guile / Majestic — Planned (requires guile-fibers)
```nu
